from typing import TypeVar, Union, Sequence, Iterable, Optional, Callable, List, Mapping, Any, Dict, Type
import os
...
def get_progress(
    ...
    """
    disable = disable or os.getenv("TQDM_DISABLE", False)
    ...
    columns.extend(
        (
            ...
            PostFixColumn(justify="left"),  # TODO: this creates a space when not postfix
...
if __name__ == '__main__':
    ...
    iterator = track(range(100))

    for x in iterator:
        time.sleep(0.1)
        iterator.set_postfix({'R@1': x, 'mAP': x / 2})
        iterator.set_description(f'Epoch {x}')
