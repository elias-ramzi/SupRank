from typing import Mapping, Optional, List, Callable

import numpy as np

import suprank.lib as lib
from suprank.engine.metrics import METRICS_DICT


def weighted_mean_hook(metrics: Mapping[str, float], weights: Optional[List[int]] = None, name: str = 'AP') -> float:
    to_avg = {}
...
def inter_set_overall_accuracy_hook(metrics: Mapping[str, float], look_up: Mapping[str, Callable] = METRICS_DICT) -> Mapping[str, float]:
    """
...
def intra_set_overall_accuracy_hook(metrics: Mapping[str, float], look_up: Mapping[str, Callable] = METRICS_DICT) -> Mapping[str, float]:
    """
...
def overall_accuracy_hook(metrics: Mapping[str, float], look_up: Mapping[str, Callable] = METRICS_DICT) -> Mapping[str, float]:
    names = metrics.keys()
